import { mkdtemp, rm } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { join, resolve } from 'node:path';
import { createInterface } from 'node:readline';
import { createFileTask, hasFailed } from '@vitest/runner/utils';
import { execa } from 'execa';
import { compileTestFile } from './compiler.js';
const EVENT_PREFIX = 'VIDOT ';
const activeCancellations = new Set();
let cancellationHandlerRegistered = false;
export async function runViDot(method, state, options) {
    const outputRoot = await mkdtemp(join(tmpdir(), 'vidot-'));
    try {
        const files = await compileFiles(state, outputRoot, options.projectPath);
        const filesByPath = new Map(files.map(file => [file.scriptPath, file]));
        let finished = false;
        const launch = options.launch({ method });
        const testNamePattern = state.config.testNamePattern;
        const args = [
            ...launch.args,
            '--path',
            options.projectPath,
            '--script',
            options.runnerPath,
            '--',
            ...(method === 'collect' ? ['--vidot-collect'] : []),
            ...(testNamePattern
                ? [`--vidot-test-name-pattern=${testNamePattern.source}`]
                : []),
            ...files.flatMap(file => [
                `--vidot-test-name=${file.file.name}`,
                `--vidot-test=${file.scriptPath}`,
            ]),
        ];
        await launch.before?.();
        const child = execa(options.godotPath, args, {
            env: launch.env,
            reject: false,
            stderr: 'inherit',
            stdout: 'pipe',
        });
        const cancel = () => child.kill();
        activeCancellations.add(cancel);
        if (!cancellationHandlerRegistered) {
            state.onCancel(() => {
                for (const cancelActiveRun of activeCancellations)
                    cancelActiveRun();
            });
            cancellationHandlerRegistered = true;
        }
        try {
            const lines = createInterface({ input: child.stdout });
            for await (const line of lines) {
                if (!line.startsWith(EVENT_PREFIX)) {
                    await state.rpc.onUserConsoleLog({
                        type: 'stdout',
                        content: `${line}\n`,
                        time: Date.now(),
                        size: 1,
                    });
                    continue;
                }
                const event = JSON.parse(line.slice(EVENT_PREFIX.length));
                if (event.type === 'run_finish') {
                    finished = true;
                    continue;
                }
                if (event.type === 'runner_error')
                    throw new Error(event.message);
                const file = filesByPath.get(resolve(event.file));
                if (!file)
                    throw new Error(`Godot reported an unknown test file: ${event.file}`);
                await applyEvent(method, state, file, event);
            }
            const result = await child;
            if (result.exitCode !== 0)
                throw new Error(`Godot exited with code ${result.exitCode ?? 'unknown'}`);
            if (!finished)
                throw new Error('Godot exited without a run_finish event');
            const incomplete = files.filter(file => (method === 'collect' ? !file.collected : !file.finished));
            if (incomplete.length > 0) {
                throw new Error(`Godot did not finish: ${incomplete.map(file => file.file.filepath).join(', ')}`);
            }
            await launch.after?.();
        }
        finally {
            activeCancellations.delete(cancel);
            child.kill();
            await child;
        }
    }
    finally {
        await rm(outputRoot, { force: true, recursive: true });
    }
}
async function compileFiles(state, outputRoot, projectPath) {
    const files = [];
    for (const [index, specification] of state.ctx.files.entries()) {
        const outputDirectory = join(outputRoot, String(index));
        const scriptPath = await compileTestFile({
            sourcePath: specification.filepath,
            outputDirectory,
            projectPath,
        });
        const file = createFile(specification.filepath, state.config.root, state.config.name);
        // Worker messages normally deserialize into distinct task objects. The in-process
        // transport must preserve that boundary so Vitest indexes collected children.
        // `state.rpc` is Vitest's official worker/reporting channel. Godot itself
        // communicates with ViDot only through the structured stdout events above.
        await state.rpc.onQueued(createFile(specification.filepath, state.config.root, state.config.name));
        files.push({
            scriptPath: resolve(scriptPath),
            file,
            tests: new Map(),
            collected: false,
            finished: false,
        });
    }
    return files;
}
function createFile(filepath, root, projectName) {
    return createFileTask(filepath, root, projectName, 'vidot', 'ssr');
}
async function applyEvent(method, state, runFile, event) {
    switch (event.type) {
        case 'file_collected':
            await reportCollection(method, state, runFile, event.tree);
            return;
        case 'test_start':
        case 'test_finish':
            await reportTestEvent(state, runFile, event);
            return;
        case 'file_finish':
            await reportFileFinish(state, runFile, event.duration, event.errors);
    }
}
async function reportCollection(method, state, runFile, tree) {
    const file = runFile.file;
    file.mode = tree.mode;
    file.tasks = tree.children.map((node, index) => createTask(state, runFile, file, node, index));
    file.collectDuration = 0;
    runFile.collected = true;
    await state.rpc.onCollected([file]);
    if (method === 'run' && file.mode === 'run') {
        file.result = { state: 'run', startTime: Date.now() };
        await state.rpc.onTaskUpdate([[file.id, file.result, file.meta]], [[file.id, 'suite-prepare', undefined]]);
    }
}
function createTask(state, runFile, parent, node, index) {
    const file = runFile.file;
    const fullTestName = parent.fullTestName
        ? `${parent.fullTestName} > ${node.name}`
        : node.name;
    const base = {
        id: `${parent.id}_${index}`,
        name: node.name,
        fullName: `${file.name} > ${fullTestName}`,
        fullTestName,
        mode: node.mode,
        meta: {},
        file,
        suite: parent,
    };
    if (node.type === 'suite') {
        const suite = {
            ...base,
            type: 'suite',
            tasks: [],
        };
        suite.tasks = node.children.map((child, childIndex) => createTask(state, runFile, suite, child, childIndex));
        return suite;
    }
    const test = {
        ...base,
        type: 'test',
        context: {},
        timeout: state.config.testTimeout,
        annotations: [],
        artifacts: [],
    };
    runFile.tests.set(node.id, test);
    return test;
}
async function reportTestEvent(state, runFile, event) {
    const test = runFile.tests.get(event.id);
    if (!test)
        throw new Error(`Godot reported an unknown test: ${event.id}`);
    test.result = event.type === 'test_start'
        ? { state: 'run', startTime: Date.now() }
        : {
            state: event.state,
            startTime: test.result?.startTime,
            duration: event.duration,
            errors: event.errors,
        };
    await state.rpc.onTaskUpdate([[test.id, test.result, test.meta]], [[test.id, event.type === 'test_start' ? 'test-prepare' : 'test-finished', undefined]]);
}
async function reportFileFinish(state, runFile, duration, errors) {
    const file = runFile.file;
    if (!runFile.collected)
        throw new Error(`Godot finished an uncollected file: ${file.filepath}`);
    file.result = {
        state: errors?.length || hasFailed(file)
            ? 'fail'
            : file.mode === 'skip' ? 'skip' : 'pass',
        duration,
        errors,
    };
    await state.rpc.onTaskUpdate([[file.id, file.result, file.meta]], [[file.id, 'suite-finished', undefined]]);
    runFile.finished = true;
}
