...
experimental. The tested Godot API baseline is 4.7.2.
...
`runtime/runner.gd` and compiles the Node adapter to `dist/`. The Godot runner
is tracked. The Node adapter is generated by the package's `prepare` script
for npm releases and Git installs. Tests use the packaged runner rather than
compiling it during a run.
